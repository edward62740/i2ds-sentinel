...
static SE_Response_t se_manager_command_response = SLI_SE_RESPONSE_█████████_ERROR;
...
sl_status_t sli_se_execute_and_wait(sl_se_command_context_t *cmd_ctx)
{
  ...
  if (cmd_ctx->yield) {
    ...
    // Default to an error.
    se_manager_command_response = SLI_SE_RESPONSE_█████████_ERROR;
  } else {
