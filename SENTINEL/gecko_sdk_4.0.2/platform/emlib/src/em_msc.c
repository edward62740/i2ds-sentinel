...
MSC_Status_TypeDef MSC_MassErase(void)
{
  ...
  MSC->WRITECMD         = MSC_WRITECMD_ERASE█████████0;            // Start Mass erase procedure
...
MSC_Status_TypeDef MSC_MassErase(void)
{
  ...
  MSC->WRITECMD = MSC_WRITECMD_ERASE█████████0;

  /* Waiting for erase to complete. */
  while ((MSC->STATUS & MSC_STATUS_BUSY) != 0U) {
  }

#if ((FLASH_SIZE >= (512 * 1024)) && defined(_MSC_WRITECMD_ERASE█████████1_MASK))
  /* Erase the second 512 K block. */
  MSC->WRITECMD = MSC_WRITECMD_ERASE█████████1;
