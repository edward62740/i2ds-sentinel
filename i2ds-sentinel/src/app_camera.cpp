...
/*! init()
   @brief initializes camera to detector mode
   @note
   @param void
   @return void
*/
...
uint8_t *AppCamera::captureGreyscale()
{
   ...
   // uint8_t *tmpbuf = (uint8_t *)heap_caps_malloc(50000, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
   // size_t tmpbuf_len;
...
/*! switchToJpeg()
   @brief switch camera to high-res jpeg mode, captures image and saves to SPIFFS. passes img_info_t associated with image to firebaseSendPayloadQueue.
   image name is generated from capture_counter which increments every time this function is called.
   @note
   @param &info reference to ipc_warn_t to assign to img_info_t
   @return String jpeg path debug
*/
String AppCamera::switchToJPEG(ipc_warn_t &info, bool init)
{
   if (init)
   {
      esp_camera_deinit();
   ...
   esp_camera_fb_return(tmpFb);
   this->capture_counter++;
   return filename;
}

/*! switchToGreyscale()
   @brief initializes camera to detector mode after taking jpeg
   @note
   @param void
   @return void
*/
